...
public class FormattedTextPresenter : UserControl
{
    ...
    private static readonly IBrush s_headerLight = new ImmutableSolidColorBrush(
        Color.FromRgb(25, 25, 112)
    );
    private static readonly IBrush s_headerDark = new ImmutableSolidColorBrush(
        Color.FromRgb(65, 105, 225)
    );
    private static readonly IBrush s_addedLight = Brushes.Green;
    private static readonly IBrush s_addedDark = new ImmutableSolidColorBrush(
        Color.FromRgb(80, 200, 120)
    );
    private static readonly IBrush s_removedLight = Brushes.Red;
    private static readonly IBrush s_removedDark = new ImmutableSolidColorBrush(
        Color.FromRgb(255, 100, 100)
    );
    private static readonly IBrush s_fileNameLight = new ImmutableSolidColorBrush(
        Color.FromRgb(255, 140, 0)
    );
    private static readonly IBrush s_fileNameDark = new ImmutableSolidColorBrush(
        Color.FromRgb(255, 180, 70)
    );
    private static readonly IBrush s_fileNameMuted = new ImmutableSolidColorBrush(
        Color.FromRgb(150, 150, 150)
    );
    private static readonly IBrush s_defaultLight = Brushes.Black;
    private static readonly IBrush s_defaultDark = new ImmutableSolidColorBrush(
        Color.FromRgb(220, 220, 220)
    );
    private static readonly IBrush s_highlightLight = Brushes.Yellow;
    private static readonly IBrush s_highlightDark = new ImmutableSolidColorBrush(
        Color.FromRgb(80, 80, 0)
    );
    ...
    public FormattedTextPresenter()
    {
        _itemsControl = new ItemsControl
        {
            ...
            ItemTemplate = new FuncDataTemplate<List<FormattedLine>>(
                BuildSectionBlock,
        ...
        _itemsControl.Template = new FuncControlTemplate<ItemsControl>(
            (_, scope) =>
            {
    ...
    {
        DocumentProperty.Changed.AddClassHandler<FormattedTextPresenter>(
            (x, _) => x.OnDocumentChanged()
        );
        SearchTermProperty.Changed.AddClassHandler<FormattedTextPresenter>(
            (x, _) => x.UpdateRealizedItems()
        );
    }
    ...
    private void UpdateRealizedItems()
    {
        ...
        for (int i = 0; i < _itemsControl.ItemCount; i++)
        {
            ...
            if (_itemsControl.Items[i] is not List<FormattedLine> section)
                continue;
    ...
    private Control BuildSectionBlock(List<FormattedLine> section, INameScope _)
    {
        ...
        tb.Inlines = BuildSectionInlines(
            section,
            SearchTerm,
    ...
    private static InlineCollection BuildSectionInlines(
        List<FormattedLine> section,
        string? searchTerm,
        bool isDark
    )
    {
        ...
        for (int i = 0; i < section.Count; i++)
        {
            ...
            foreach (var span in section[i].Spans)
            {
                ...
                var fontSize = span.Color is SpanColor.FileName or SpanColor.FileNameMuted
                    ? 15d
                    : 13d;

                if (
                    !string.IsNullOrEmpty(searchTerm)
                    && span.Text.Contains(searchTerm, StringComparison.OrdinalIgnoreCase)
                )
                {
                    AddHighlightedRuns(
                        inlines,
    ...
    )
    {
        ...
        while (pos < text.Length)
        {
            ...
            inlines.Add(
                new Run(text[matchIdx..(matchIdx + term.Length)])
                {
    ...
    private static IBrush ResolveColor(SpanColor color, bool isDark) =>
        color switch
        {
